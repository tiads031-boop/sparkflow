...
> **状态**：实施中；R1–R8 已合并；R9 代码实施中；终端与生产验收最后集中进行
...
实施中：默认关闭、Android Usage Access 显式授权，只读取用户选定应用的前台使用区间；独立 AppUsageSession 与标签映射；与 Focus/手工补记分开呈现和统计，防止重叠时长被加两次。生产迁移和真机验收最后集中进行。
