...
| GitHub | `master@e883b9a3`；VNext Plan M1–M3（功能基线 `7145ba0`，PR #48/#49/#50）已合并并通过 CI；PR #51 已同步方案事实源 |
...
- [ ] **M5 / AI 规划与调整 2.0**：把固定“AI 帮我安排”改为对话式助手；AI 根据需求缺口自主访谈，不限制固定问题数；保存持续 Planning Context（已确认目标/约束/偏好/策略/假设/revision）；支持文字 + 语音、临时想法/任务、增量重规划、目标替换、Preview → Apply → Undo。
- [ ] **M6 / 学习目标 AI**：Study 与 Course 完全解耦；长期目标绑定持续 Planning Context；AI 尽可能了解成功标准、当前水平、资源、时间预算、偏好与取舍，再拆阶段/里程碑/Task；后续冲突、执行效果和目标变化沿用上下文增量调整。
