...
### M1：课程状态契约与导入基础（2–3 天）

- 为课程列表和课表概览定义统一状态：`idle/loading/success/error/refreshing`。
- 缓存已有数据；刷新失败时保留旧数据并显示非阻断提示。
- 请求支持 AbortController、统一超时和学期/账号切换后的过期响应隔离。
- 将解析、时间归一化、字段校验、预览转换拆成纯函数。
- 接受 `8:00`、`08:00`、中文冒号与常见连接符，内部统一为 `HH:mm`。
