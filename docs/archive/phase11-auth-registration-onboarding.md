...
> **创建时间**: 2026-06-04 | **归档时间**: 2026-09-15 | **状态**: ✅ 已完成
...
本 Phase 记录的是早期本地账户实现。2026-09-13 起，生产认证已迁移为 SparkFlow API 自建认证：服务端保存密码哈希与会话，前端只保存 bearer token；本地 profile/onboarding 偏好仍按用户 ID 存储。历史实现已被替代，不再作为生产认证事实源。
