...
> **创建时间**: 2026-05-06 | **最后更新**: 2026-05-29（项目代办自动折叠 + @双向链接） | **状态**: Phase 7 学期系统开发完成
...
| 29 | **项目代办自动折叠** | 进入看板时，没有 `In progress` 任务的项目分组自动折叠；`useRef` 标记首次执行，不覆盖用户手动操作 | 减少视觉噪音，聚焦正在推进的项目；"其他"始终展开保证信息可见性 |
| 30 | **@双向链接** | 任务描述中用 `@项目名` / `@任务标题` 建立双向链接；TaskCard 显示 mention 彩色标签；DarkFrostedModal 编辑时实时预览匹配；BoardView 项目头显示 @回链计数 | 项目与任务之间形成可追溯的引用网络；纯前端计算，不改 md 协议和 DB schema |
...
### 2026-05-29（项目代办自动折叠 + @双向链接）
- **项目代办自动折叠**：进入看板时，无 `In progress` 任务的项目分组自动折叠
  - `useRef<boolean>(false)` 标记 `hasAutoCollapsed`，首次渲染后不再触发
  - "其他"分组始终展开
  - 用户手动切换折叠不受自动逻辑覆盖
- **@双向链接**：任务描述中用 `@项目名` / `@任务标题` 建立双向引用
  - 新建 `utils/mentionUtils.ts`：`extractMentions()` / `resolveMentions()` / `renderMentionText()`
  - `TaskCard`：描述中的 mention 渲染为可点击彩色标签（项目=紫色 #b0a8db，任务=绿色 #cae393），最多显示 3 个
  - `DarkFrostedModal`：编辑描述时 textarea 下方实时预览匹配到的项目和任务
  - `BoardView`：项目分组头显示 @回链计数（非本组任务 @该项目名的次数）
  - 纯前端计算，不改 md 协议和 DB schema
