          ...
          {items.map((item) => {
            ...
            return (
              <button key={item.id} type="button" onClick={() => onItemClick(item)} className="relative grid w-full grid-cols-[38px_10px_minmax(0,1fr)] items-start gap-2 text-left">
                ...
                <span className="relative z-10 mt-1.5 grid h-2.5 w-2.5 place-items-center rounded-full border-2 border-[var(--sf-bg)]" style={{ backgroundColor: item.color }}><Icon size={0} aria-hidden="true" /></span>
