...
export default function RecordCardsView({ records, loading, onOpen, onRefresh }: {
  ...
  return (
    <section className="space-y-3">
      <div className="flex items-center justify-between text-xs text-[var(--sf-text-tertiary)]"><span>{records.length} 条记录 · {reflectionCount} 条 Reflection</span><button type="button" onClick={onRefresh} className="flex items-center gap-1"><RefreshCw size={12} /> 刷新</button></div>
      ...
      {records.map((record) => (
        <article key={record.id} className="rounded-[22px] border border-[var(--sf-border)] bg-[var(--sf-surface)] p-4 shadow-[0_8px_20px_rgba(25,31,28,0.05)]">
          <div className="mb-3 flex items-start justify-between gap-3"><div className="min-w-0"><p className="text-[11px] text-[var(--sf-text-tertiary)]">{new Date(record.createdAt).toLocaleString('zh-CN', { month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit' })} · {recordSourceLabel(record)}</p><p className="mt-2 whitespace-pre-wrap text-[11px] font-medium leading-5 text-[var(--sf-text-primary)]">{recordText(record)}</p></div><button type="button" onClick={() => onOpen(record)} className="shrink-0 rounded-full bg-[var(--sf-bg)] px-3 py-2 text-[10px] font-bold text-[var(--sf-text-secondary)]">查看</button></div>
