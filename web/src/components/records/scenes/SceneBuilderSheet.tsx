...
export default function SceneBuilderSheet({ scene, onClose, onSaved }: { scene?: SceneTemplate; onClose: () => void; onSaved: (scene: SceneTemplate) => void }) {
  ...
  return <div className="fixed inset-0 z-[70] flex items-end justify-center bg-black/40" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <section role="dialog" aria-modal="true" aria-label={scene ? '编辑场景' : '创建场景'} className="max-h-[90dvh] w-full max-w-xl overflow-y-auto rounded-t-[30px] bg-[var(--sf-bg)] p-5 pb-[calc(env(safe-area-inset-bottom)+24px)] text-[var(--sf-text-primary)] shadow-xl">
      ...
      <fieldset className="mt-5"><legend className="text-sm font-bold">触发方式</legend><div className="mt-2 flex flex-wrap gap-2">{Object.entries(triggerNames).map(([key, label]) => <label key={key} className="rounded-full bg-white px-3 py-2 text-xs"><input type="checkbox" disabled={key === 'task_completed'} checked={triggers.includes(key)} onChange={() => setTriggers((current) => current.includes(key) ? current.filter((item) => item !== key) : [...current, key])} className="mr-2" />{label}{key === 'task_completed' ? ' · 即将支持' : ''}</label>)}</div><p className="mt-2 text-[11px] text-[var(--sf-text-secondary)]">自动关联 Focus 需在“我的 → 时间记录”选择此场景为默认场景。</p></fieldset>
      ...
      <fieldset className="mt-5"><legend className="text-sm font-bold">允许视图</legend><div className="mt-2 flex flex-wrap gap-2">{Object.entries(viewNames).map(([key, label]) => <label key={key} className="rounded-full bg-white px-3 py-2 text-xs"><input type="checkbox" checked={views.includes(key as keyof typeof viewNames)} onChange={() => setViews((current) => current.includes(key as keyof typeof viewNames) ? current.filter((item) => item !== key) : [...current, key as keyof typeof viewNames])} className="mr-2" />{label}</label>)}</div></fieldset>
      {error ? <p role="alert" className="mt-4 text-xs text-red-600">{error}</p> : null}
      <button type="button" onClick={() => void save()} disabled={busy || !name.trim() || !triggers.length || !views.length} className="mt-6 w-full rounded-2xl bg-[var(--sf-graphite)] p-4 text-sm font-bold text-white disabled:opacity-50">{busy ? '保存中…' : '保存场景'}</button>
    </section>
  </div>;
}
