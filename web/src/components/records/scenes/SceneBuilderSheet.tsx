...
export default function SceneBuilderSheet({ scene, onClose, onSaved }: { scene?: SceneTemplate; onClose: () => void; onSaved: (scene: SceneTemplate) => void }) {
  ...
  return <div className="fixed inset-0 z-[70] flex items-end justify-center bg-black/40" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <section role="dialog" aria-modal="true" aria-label={scene ? '编辑场景' : '创建场景'} className="max-h-[90dvh] w-full max-w-xl overflow-y-auto rounded-t-[30px] border border-[var(--sf-border)] bg-[var(--sf-surface)] p-5 pb-[calc(env(safe-area-inset-bottom)+24px)] text-[var(--sf-text-primary)] shadow-xl">
      ...
      <div className="mt-5 grid grid-cols-[5rem_1fr] gap-3">
        <label className="text-xs">图标<input aria-label="场景图标" value={emoji} maxLength={12} onChange={(event) => setEmoji(event.target.value)} className="mt-1 w-full rounded-xl border border-[var(--sf-border)] bg-[var(--sf-bg)] p-3" /></label>
        <label className="text-xs">名称<input required value={name} maxLength={60} onChange={(event) => setName(event.target.value)} className="mt-1 w-full rounded-xl border border-[var(--sf-border)] bg-[var(--sf-bg)] p-3" placeholder="例如：阅读" /></label>
      </div>
      ...
      <fieldset className="mt-5"><legend className="text-sm font-bold">触发方式</legend><div className="mt-2 flex flex-wrap gap-2">{Object.entries(triggerNames).map(([key, label]) => <label key={key} className="rounded-full bg-[var(--sf-bg)] px-3 py-2 text-xs"><input type="checkbox" disabled={key === 'task_completed'} checked={triggers.includes(key)} onChange={() => setTriggers((current) => current.includes(key) ? current.filter((item) => item !== key) : [...current, key])} className="mr-2" />{label}{key === 'task_completed' ? ' · 即将支持' : ''}</label>)}</div><p className="mt-2 text-[11px] text-[var(--sf-text-secondary)]">自动关联 Focus 需在“我的 → 时间记录”选择此场景为默认场景。</p></fieldset>
