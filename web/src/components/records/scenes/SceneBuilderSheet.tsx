...
export default function SceneBuilderSheet({ scene, onClose, onSaved }: { scene?: SceneTemplate; onClose: () => void; onSaved: (scene: SceneTemplate) => void }) {
  ...
  return <div className="fixed inset-0 z-[70] flex items-end justify-center bg-black/40" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <section role="dialog" aria-modal="true" aria-label={scene ? '编辑场景' : '创建场景'} className="max-h-[90dvh] w-full max-w-xl overflow-y-auto rounded-t-[30px] bg-[var(--sf-bg)] p-5 pb-[calc(env(safe-area-inset-bottom)+24px)] text-[var(--sf-text-primary)] shadow-xl">
      ...
      <fieldset className="mt-5"><legend className="text-sm font-bold">触发方式</legend><div className="mt-2 flex flex-wrap gap-2">{Object.entries(triggerNames).map(([key, label]) => <label key={key} className="rounded-full bg-white px-3 py-2 text-xs"><input type="checkbox" disabled={key !== 'manual'} checked={triggers.includes(key)} onChange={() => setTriggers((current) => current.includes(key) ? current.filter((item) => item !== key) : [...current, key])} className="mr-2" />{label}{key !== 'manual' ? ' · 即将支持' : ''}</label>)}</div></fieldset>
