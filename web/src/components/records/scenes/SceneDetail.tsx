...
export default function SceneDetail({ scene, onBack, onEdit }: { scene: SceneTemplate; onBack: () => void; onEdit: () => void }) {
  ...
  return <section className="space-y-4 pb-5">
    ...
    {!loading && activeView === 'heatmap' ? <div className="rounded-[24px] bg-white p-4"><h3 className="mb-3 text-xs font-bold">记录热力图{scene.allowedViews.includes('list') ? ' · 点击日期查看' : ''}</h3><div className={period === 'year' ? 'grid grid-cols-[repeat(14,minmax(0,1fr))] gap-1' : 'grid grid-cols-7 gap-1.5'}>{heatmapDays.map((day) => <button type="button" key={day.date} disabled={!scene.allowedViews.includes('list')} title={`${day.date} · ${day.count} 条`} aria-label={`${day.date} ${day.count} 条${scene.allowedViews.includes('list') ? '，查看详情' : ''}`} onClick={() => { setCustomStart(day.date); setCustomEnd(day.date); setPeriod('custom'); setView('list'); }} className="aspect-square rounded-md" style={{ backgroundColor: scene.color, opacity: day.count ? 0.25 + day.count / maxCount * 0.75 : 0.12 }} />)}</div></div> : null}
