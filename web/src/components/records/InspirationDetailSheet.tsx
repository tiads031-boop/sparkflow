  ...
  return createPortal(
    <div className="fixed inset-0 z-[120] flex items-end justify-center bg-black/30" role="presentation" onClick={() => !busy && onClose()}>
      <section
        ...
        aria-modal="true"
        aria-label="记录详情"
        className="flex max-h-[88dvh] w-full max-w-lg flex-col rounded-t-[2rem] bg-[var(--sf-surface)] shadow-2xl"
        onClick={(event) => event.stopPropagation()}
      >
        <header className="flex shrink-0 items-center justify-between border-b border-[var(--sf-border)] px-5 py-4">
          <div>
        ...
        <div className="min-h-0 flex-1 space-y-4 overflow-y-auto px-5 py-4">
          <label className="block">
            ...
            <textarea
              ...
              className="mt-2 min-h-36 w-full resize-none rounded-2xl border border-[var(--sf-border)] bg-[var(--sf-bg)] px-4 py-3 text-sm leading-6 outline-none focus:border-[var(--sf-text-primary)]"
            />
          </label>
        ...
        <footer className="grid shrink-0 grid-cols-[auto_1fr] gap-2 border-t border-[var(--sf-border)] px-5 pb-[calc(env(safe-area-inset-bottom,0px)+16px)] pt-3">
