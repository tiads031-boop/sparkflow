...
import AppUsageSettings from './AppUsageSettings';
...
export default function TimeTrackingSettingsView({ onBack, onOpenActual }: { onBack: () => void; onOpenActual: () => void }) {
  ...
  return <div className="animate-page-enter space-y-4 pb-24 text-[var(--sf-text-primary)]">
    <header className="flex items-start gap-3"><button type="button" onClick={onBack} aria-label="返回我的" className="grid h-9 w-9 shrink-0 place-items-center rounded-full border border-[var(--sf-border)] bg-[var(--sf-surface)] text-sm">←</button><div><h1 className="text-xl font-black">时间记录</h1><p className="mt-1 text-[10px] text-[var(--sf-text-secondary)]">决定 Actual Timeline 的数据来源</p></div></header>
    ...
    {preferences ? <>
      <section className="overflow-hidden rounded-[22px] border border-[var(--sf-border)] bg-[var(--sf-surface)] px-4 shadow-sm">{switches.map(({ key, title, description }) => <label key={key} className="flex min-w-0 items-center justify-between gap-3 border-b border-[var(--sf-border)] py-3 last:border-0"><span className="min-w-0 flex-1"><strong className="block text-[11px]">{title}</strong><span className="mt-1 block text-[9px] leading-4 text-[var(--sf-text-secondary)]">{description}</span></span><input type="checkbox" checked={preferences[key]} disabled={busy} onChange={(event) => void save({ [key]: event.target.checked })} aria-label={title} className="peer sr-only" /><span aria-hidden="true" className="relative h-6 w-11 shrink-0 rounded-full bg-[#e2e5e2] transition peer-checked:bg-[var(--sf-green-strong)] after:absolute after:left-[3px] after:top-[3px] after:h-[18px] after:w-[18px] after:rounded-full after:bg-white after:shadow-sm after:transition-transform peer-checked:after:translate-x-[18px]" /></label>)}</section>
      <section className="rounded-[22px] border border-[var(--sf-border)] bg-[var(--sf-surface)] p-4 shadow-sm"><label className="block text-sm font-bold">默认场景<select value={preferences.defaultSceneId ?? ''} disabled={busy} onChange={(event) => void save({ defaultSceneId: event.target.value || null })} className="mt-3 w-full rounded-xl bg-[var(--sf-bg)] p-3 text-sm font-normal"><option value="">不自动关联</option>{scenes.map((scene) => <option key={scene.id} value={scene.id}>{scene.emoji} {scene.name}{scene.triggers.includes('focus') ? '' : '（未开启专注触发）'}</option>)}</select></label><p className="mt-2 text-xs leading-5 text-[var(--sf-text-secondary)]">新完成的 Focus 仅在该场景开启“专注”触发时自动关联；带必填字段的记录可之后补全。</p></section>
      <AppUsageSettings />
