...
function toApiPayload(task: Partial<Task> & { title?: string }): Record<string, unknown> {
  ...
  // inspirationId / insightId are intentionally read-only in the generic task editor.
  // Phase 15 source links are only created by dedicated user-scoped confirmation endpoints.
